...
struct LibJpeg:
    ...
    fn from_path(self, filename : Path, dimensions : ParamsDimensions) raises -> JpegImage:
        ...
        if filename.is_file() and self._good:
            ...
            with open(filename, "rb") as f:
                ...
                bytes = f.read_bytes()   
            if bytes.size>0:
                if not self.decompress(bytes, dimensions, result):
                    result.clear()
        return result

    # to be complete, we'll need to add the colorspace and the rotation (need EXIF for that)
    ...
    fn compress(self, bytes : DTypePointer[DType.uint8, AddressSpace.GENERIC], width : UInt32, height : UInt32, params : ParamsCompression, icc_profile : List[SIMD[DType.uint8,1]]) -> List[SIMD[DType.uint8,1]]:
        var num_bytes = (width*height*BPP).cast[DType.int64]().value
        var buffer_dest =  List[SIMD[DType.uint8,1]](capacity=num_bytes)
        if self.is_ok() and width>0 and height>0: # rgbx32 mandatory, for now
            # to avoid any king of memory trouble, I work with a buffer sized as a uncompressed image and will downsized it later.
    ...
    fn to_path(self, file_name : Path, img : JpegImage, params : ParamsCompression) raises -> Bool:
        var result = False
        if self.is_ok():
            var filename = set_extension(file_name,"jpg")
            var bytes = self.compress(img.pixels, img.get_width(), img.get_height(), params, img.icc)
            var t = bytes.size
...
struct JpegImage(Stringable):
    ...
    fn set_dimensions(inout self, width : UInt32, height : UInt32, full_width : UInt32, full_height : UInt32):
        ...
        if self._num_bytes>0:
            self.pixels.free()
        self._num_bytes = Int(width.cast[DType.int32]().value)*Int(height.cast[DType.int32]().value)*4  # working with UInt32 is painfull
        self.pixels = self.pixels.alloc(self._num_bytes) # I should be smarter than that and reuse existing memory if possible
        self._width = width
        self._height = height
    ...
    fn to_ppm(self, x : Path) raises -> Bool:
        ...
        for adr in range(num_pixels): 
            var rgba = self.pixels.load[width=4](adr*self.bpp)
    ...
    fn to_jpeg(self, filename : Path, params : ParamsCompression, libjpeg : LibJpeg) raises -> Bool:
        """ 
            save the image as a jpeg file.
        """        
        var result = libjpeg.is_ok()
        if result:
            result = libjpeg.to_path(filename, self, params)
        return result

    @staticmethod
    fn from_jpeg(filename : Path, params : ParamsDimensions, libjpeg : LibJpeg) raises -> Optional[Self]:
        """ 
            load the image as a jpeg file.
        """        
        var result = Optional(None)
        if libjpeg.is_ok():
            var img = libjpeg.from_path(Path("test/image_icc.jpg"), params)
            result = Optional(img)
        return result

    fn convert_to_icc_profile(inout self)  -> Bool:
        var result = True
        """ 
        to be done in a near future. Need to "FFI" lcms2 for that
        """
        return result
...
fn validation_libjpeg() raises :
    ...
    img = libjpeg.from_path(Path("test/image_icc.jpg"), dim)
    ...
    var params = ParamsCompression(98,False)
    result = img.to_jpeg(Path("test/image_tempo.jpg"), params, libjpeg)
    assert_true(result)
